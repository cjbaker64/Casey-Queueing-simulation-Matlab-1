...
%%
%[text] ## Part A: Questions
%[text] #### 1. From the theory for an M/M/1 queue, compute the following steady-state parameters. 
%[text]     a. 𝐿: expected total number of customers in the system = 5.0000
%[text]     b. 𝐿𝑞: expected number of customers waiting (not being served) = 4.1667
%[text]     c. 𝑊: expected total time in the system, waiting plus service time = 0.5000
%[text]     d. 𝑊𝑞: expected waiting time in the queue = 0.4167
L = rho / (1-rho)
L_q = (rho)^2 / (1-rho)
W = 1 / (mu-lambda)
W_q = rho / (mu-lambda)
%[text] #### 2. Run the given simulation script. How well do the theoretical results agree with the statistics calculated from the simulation? Express the discrepancy as a percentage of the theoretical value. 
%[text]     a. 𝐿 discrepancy = `3.8266%`
%[text]     b. 𝐿𝑞 discrepancy = `4.4961%`
%[text]     c. *W* discrepancy = `3.9473%`
%[text]     d. 𝑊𝑞 discrepancy = `4.6978`%
%[text] According to these results, the simulation statistics and the theoretical results are very close in agreement while not exactly the same (between 3% and 5% discrepancy). 
%[text] 
%[text] Adding 𝐿𝑞 and W𝑞 estimates, as in lines 52-66:
...
%[text] Compute discrepancies:
L_disc = abs(meanNumInSystem - L)/L * 100
L_q_disc = abs(meanNumWaiting - L_q)/L_q * 100
W_disc = abs(meanTimeInSystem - W)/W * 100
W_q__disc = abs(meanWaitingTime - W_q)/W_q * 100
%%
%[text] ## 
...
%---
%[output:6fc45b12]
%   data: {"dataType":"error","outputData":{"errorType":"runtime","text":"servers requires <a href=\"matlab:matlab.internal.addons.launchers.showExplorer('ErrorRecovery', 'identifier', 'MG', 'focused', 'servers');\">Mapping Toolbox<\/a>."}}
%---
